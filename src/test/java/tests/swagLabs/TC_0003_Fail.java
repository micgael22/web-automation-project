...
public class TC_0003_Fail extends BaseController {
    ...
    @Story("Swag Labs")
    @Test(groups = {"test FAIL"}, description = "TC_ID=04001 | Verify text not present")
    public void verify_item_fail() throws Exception {
        ...
        uiAction.executeStep("Verify (Red T-Shirt) is present", "VERIFY_ELEMENT_TEXT", poCheck.itemTitle00, "Test.allTheThings() T-Shirt (black)");
    }

    @Description("locator not present")
    @Severity(SeverityLevel.MINOR)
    @Story("Swag Labs")
    @Test(groups = {"test FAIL"}, description = "TC_ID=04002 | Unable to click on locator")
    ...
    @Severity(SeverityLevel.MINOR)
    @Story("Swag Labs")
    @Test(groups = {"test FAIL"}, description = "TC_ID=04003 | Verify incorrect URL")
